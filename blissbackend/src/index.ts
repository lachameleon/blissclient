...
export class ChatRoom extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/sightings") {
      ...
      return this.handleSightingPost(request);
    }

    return new Response("Not found", { status: 404 });
  }

  private async handleSightingPost(request: Request): Promise<Response> {
    ...
    const reporter = sanitizeReporter(payload);
    if (!reporter) return new Response(null, { status: 204, headers: CORS_HEADERS });
...
function sanitizeReporter(payload: JsonObject): ReporterIdentity | null {
  const username = sanitizeUsername(payload.reporterUsername);
  const uuid = sanitizeUuid(payload.reporterUuid);
  const serverAddress = sanitizeServerAddress(payload.reporterServerAddress);
  if (!username || !uuid || !serverAddress) return null;
  return { username, uuid, serverAddress };
}
    ...
    <section class="app">
      <div class="workspace">
        <header class="topbar">
          <div class="brand">
            ...
            <div>
              ...
              <p class="eyeline">Shared player tracker telemetry from Bliss Client sessions.</p>
            </div>
          </div>
        ...
        <section class="metrics" aria-label="Bliss client metrics">
          <article class="metric feature online">
            <div class="metric-label">Active reporters</div>
            <div class="metric-value" id="onlinePlayers">0</div>
            <div class="metric-note" id="onlineNote">Waiting for tracker uploads</div>
          </article>
          <article class="metric">
            <div class="metric-label">Active servers</div>
            <div class="metric-value" id="onlineServers">0</div>
            <div class="metric-note">Reporting in the last 5 minutes</div>
          </article>
          <article class="metric stats">
          ...
          <article class="metric">
            <div class="metric-label">Seen / 15 min</div>
            <div class="metric-value" id="sightingsLast15m">0</div>
            <div class="metric-note">Recent tracker velocity</div>
          </article>
          <article class="metric">
            <div class="metric-label">Reporter profiles</div>
            <div class="metric-value" id="uniquePlayers">0</div>
            <div class="metric-note">Unique uploaders</div>
          </article>
          <article class="metric">
          ...
          <article class="metric">
            <div class="metric-label">Reporter/server pairs</div>
            <div class="metric-value" id="totalSessions">0</div>
            <div class="metric-note">Distinct upload contexts</div>
          </article>
          <article class="metric">
            <div class="metric-label">Reporters / hour</div>
            <div class="metric-value" id="joinsLastHour">0</div>
            <div class="metric-note">Recently active clients</div>
          </article>
          <article class="metric">
        ...
        <section class="dashboard-grid">
          <article class="panel">
            <div class="panel-header">
              <div>
                <h2>Server Mix</h2>
                <p class="panel-sub">Most common servers seen from tracker uploads.</p>
              </div>
              <span class="chip" id="serverChip">0 live</span>
            </div>
            <div class="server-list" id="servers">
              <div class="empty">No server reports yet.</div>
            </div>
          </article>
          ...
          <article class="panel">
            <div class="panel-header">
              <div>
                ...
                <p class="panel-sub">Recent sighting distribution from tracker uploads.</p>
              </div>
              <span class="chip" id="activityChip">0 reports</span>
            </div>
      ...
      <aside class="side">
        <article class="panel">
          <div class="panel-header">
            <div>
              <h2>Active Reporting</h2>
              <p class="panel-sub">Recent uploaders inferred from tracker reports.</p>
            </div>
            <span class="chip" id="onlineChip">0 online</span>
          </div>
          <div class="users" id="users">
            <div class="empty">No active reporters.</div>
          </div>
        </article>
        ...
        <article class="panel">
          <div class="panel-header">
            <div>
              ...
              <p class="panel-sub">Tracker freshness and server spread from the Worker.</p>
            </div>
          </div>
          <div class="server-list">
            ...
            <div class="server-row">
              <div class="server-main">
                <div class="server-name">Report recency</div>
  ...
  <script>
    ...
    function renderUsers(sightings) {
      ...
      for (const sighting of sightings) {
        if (number(sighting.timestamp) < activeSince) continue;
        const reporter = sighting.reporter || "unknown";
        if (!reporters.has(reporter)) {
          reporters.set(reporter, {
            username: reporter,
            serverAddress: sighting.serverAddress || "unknown"
          });
        }
      }

      const users = Array.from(reporters.values()).sort((a, b) => a.username.localeCompare(b.username));
      usersEl.replaceChildren();
      document.getElementById("onlineChip").textContent = users.length + " active";
      if (!users.length) {
        const empty = document.createElement("div");
        empty.className = "empty";
        empty.textContent = "No active reporters.";
    ...
    function updateStats(nextStats) {
      ...
      const activeReporters = number(stats.activeReporters);
      const activeServers = number(stats.activeServers);
      const uniquePlayers = number(stats.uniqueReporters);
      const uniqueServers = number(stats.uniqueServers);
      const reporterServerPairs = number(stats.reporterServerPairs);
      const totalSightings = number(stats.totalSightings);
      const sightingsLast15m = number(stats.sightingsLast15m);
      const sightingsLastHour = number(stats.sightingsLastHour);
      const uniqueSeenPlayers = number(stats.uniqueSeenPlayers);
      const activeReportersLastHour = number(stats.activeReportersLastHour);

      setText("onlinePlayers", formatNumber(activeReporters));
      setText("onlineServers", formatNumber(activeServers));
      setText("uniquePlayers", formatNumber(uniquePlayers));
      setText("uniqueServers", formatNumber(uniqueServers));
      setText("totalSessions", formatNumber(reporterServerPairs));
      setText("totalSightings", formatNumber(totalSightings));
      setText("sightingsLast15m", formatNumber(sightingsLast15m));
      setText("sightingsLastHour", formatNumber(sightingsLastHour));
      setText("uniqueSeenPlayers", formatNumber(uniqueSeenPlayers));
      setText("joinsLastHour", formatNumber(activeReportersLastHour));
      setText("busiestServer", stats.busiestServer || "None yet");
      setText("onlineNote", activeReporters ? activeServers + " active server" + plural(activeServers) : "Waiting for tracker uploads");
      setText("sightingNote", stats.latestSightingAt ? "Last report " + relativeTime(stats.latestSightingAt) : "No sightings yet");
      setText("lastActivity", stats.latestSightingAt ? "Latest report " + formatTime(stats.latestSightingAt) : "No activity");
      setText("generatedMetric", stats.generatedAt ? formatTime(stats.generatedAt) : "Pending");
      setText("serverChip", activeServers + " active");
    ...
    function renderServers(servers) {
      ...
      if (!servers.length) {
        const empty = document.createElement("div");
        empty.className = "empty";
        empty.textContent = "No server reports yet.";
      ...
      for (const server of servers) {
        ...
        count.textContent = formatNumber(number(server.sessions)) + " reports";
        if (number(server.online)) count.textContent += " / " + server.online + " active";
    ...
    function renderSnapshot(currentStats) {
      const active = number(currentStats.activeReporters);
      const uniquePlayers = Math.max(1, number(currentStats.uniqueReporters, active));
      const density = Math.min(100, Math.round(active / uniquePlayers * 100));
      setBar("densityBar", density);
      setText("densityText", density + "%");

      const latest = number(currentStats.latestSightingAt);
      ...
      const activeServers = number(currentStats.activeServers);
      const spread = Math.min(100, Math.round(activeServers / uniqueServers * 100));
