...
public class SignatureSign extends Module {
    public SignatureSign() { super(Categories.World, "SignatureSign", "Autofill signs with custom text."); }
    ...
    private void onScreenOpened(OpenScreenEvent event) {
        ...
        if (sh != null && sh.isActive() && sh.getRestoration(sign) != null) return;
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (!signsToColor.isEmpty() || !signsToGlowInk.isEmpty() || !signsToWax.isEmpty()) {
            if (waxAura != null && waxAura.isActive()) {
                waxAura.toggle();
        ...
        if (timer >= 5) {
            ...
            if (!signsToWax.isEmpty()) {
                ...
                if (waxAura != null && didDisableWaxAura && !waxAura.isActive()) {
                    waxAura.toggle();
                    didDisableWaxAura = false;
                }
