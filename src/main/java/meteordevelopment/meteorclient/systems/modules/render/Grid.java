...
public class Grid extends Module {
    ...
    public Grid() {
        super(Categories.Render, "grid", "Allows you to place blocks on a grid.");
    }
    ...
    private BlockPos getHeight(BlockPos blockPos) {
        // Simplified due to API differences - just return the block position
        return blockPos;
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        for (int x = -gridSize.get(); x <= gridSize.get(); x++) {
            for (int z = -gridSize.get(); z <= gridSize.get(); z++) {
                ...
                BlockPos finalBlockPos = new BlockPos(centerPos.getX() + x * gridGap.get(), centerPos.getY(), centerPos.getZ() + z * gridGap.get());
