...
public final class MapPalette {
   ...
   private static double getDistance(Color c1, Color c2) {
      ...
      return weightR * r * r + weightG * g * g + weightB * (double)(b * b);
   }
   ...
   public static BufferedImage bytesToImage(byte[] bytes, int width, int height) {
      ...
      if (pixels.length != bytes.length) {
         throw new IllegalArgumentException("Length of byte array isn't equal to resolution");
      } else {
         for(int i = 0; i < pixels.length; ++i) {
            pixels[i] = getColor(bytes[i]).getRGB();
         }
